...
def insert_files(
    ...
    # Check if any column needs content_text so we can pre-validate files
    needs_text = False
    for coldef in column:
        if ":" in coldef:
            _, coltype = coldef.rsplit(":", 1)
        else:
            coltype = coldef
        if coltype == "content_text":
            needs_text = True
            break

    # Pre-validate all text files before any database operation.
    # This ensures encoding errors are caught upfront so no partial
    # results are left in the database.
    pre_read_text = {}
    text_errors = []
    if needs_text:
        for p, relative_path in paths_and_relative_paths:
            if p == "-":
                continue  # stdin is handled separately at insert time
            resolved = pathlib.Path(p).resolve()
            try:
                pre_read_text[resolved] = resolved.read_text(encoding=encoding)
            except UnicodeDecodeError as e:
                text_errors.append((resolved, e))

        if text_errors:
            error_lines = []
            for fpath, exc in text_errors:
                error_lines.append(
    ...
    with progressbar(paths_and_relative_paths, silent=silent) as bar:
        ...
        def to_insert():
            for path, relative_path in bar:
                ...
                def _content_text(p):
                    return pre_read_text[p.resolve()]
